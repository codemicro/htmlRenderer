...
type withExternalLinkProtection struct {
	currentHost string
}

func (o *withExternalLinkProtection) SetConfig(c *renderer.Config) {
	c.Options[optExternalLinkProtection] = true
}

func (o *withExternalLinkProtection) SetHTMLOption(c *Config) {
	c.Unsafe = true
	c.CurrentHost = o.currentHost
}

// WithExternalLinkProtection renders anchors to external sites with
// rel="noopener noreferrer".
func WithExternalLinkProtection(currentHost string) interface {
	renderer.Option
	Option
} {
	return &withExternalLinkProtection{currentHost: currentHost}
}
...
func (r *Renderer) renderHeading(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	n := node.(*ast.Heading)
	if entering {
		n.SetAttributeString("class", []byte("header"))
		_, _ = w.WriteString("<h")
...
func (r *Renderer) isURLExternal(u []byte) bool {
	if len(u) == 0 {
		return false
	}
	if u[0] == '#' {
	...
	up, err := url.Parse(string(u))
	if err != nil {
		return true // just in case
	}
	if up.Scheme == "" {
		// Assuming this means we're doing a relative link
		return false
	}
	return up.Host != r.Config.CurrentHost
}
...
func (r *Renderer) renderAutoLink(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if r.Config.ExternalLinkProtection {
		if r.isURLExternal(url) && n.AutoLinkType != ast.AutoLinkEmail {
...
func (r *Renderer) renderLink(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if entering {
		...
		if r.Config.ExternalLinkProtection {
			if r.isURLExternal(n.Destination) {
				n.SetAttribute([]byte("rel"), []byte("noopener noreferrer"))
			}
		}
